...
/*    t->payload[15] = crc & 0xff;
    t->payload[16] = (crc >> 8) & 0xff;*/
    memcpy(&(t->payload[15]), &crc, 2) ;  // CRC pe primii 9 bytes*/
    printf("CRC sent is %hu\n", crc);
...
void create_D_package(msg *t, char* buffer_zone, int buffer_length, char current_SEQ){
    t->payload[0] = SOH;
...
int send_file_with_name(char argv[], char *current_SEQ){
    ...
    msg t;
    msg *answer;
    char *buffer = (void *)malloc(MAXL);
    FILE* fisier_de_copiat = fopen(argv, "rb");
    while (no_of_bytes_read){
/*        no_of_bytes_read = read (file_handler, buffer, MAXL);
        ((char*)buffer)[MAXL] = '\0';*/
        no_of_bytes_read = fread(buffer, 1, MAXL, fisier_de_copiat);
        // seems ok
        if (no_of_bytes_read) {
